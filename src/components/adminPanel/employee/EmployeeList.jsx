...
const EmployeeList = () => {
  ...
  const fetchEmployeeDetails = async () => {
    try {
      ...
      // responseData should be an object with a "response" property
      const { limitedData, totalData } = responseData.response;

      setEmployeeData(limitedData || []);
      setTotalList(totalData || 0);
    } catch (error) {
