...
function TaxYearReturn() {
  ...
  const getTaxReturnDocuments = async () => {
    const token = await getAccessToken();
    const response = await fetch(AdminAuthorURL.taxType.getTaxReturnDocuments, {
      method: "GET",
      headers: {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      setLoaderBtn(true);
      const response = await fetch(AdminAuthorURL.taxType.addTaxReturnDocument, {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
          "content-type": "application/json",
        },
        body: JSON.stringify({ name: data.taxYearReturn }),
      });
      ...
      setLoaderBtn(false);
      toast.error("Error Adding Tax Return Document");
    }
  };

  const { isLoading, isError, data, error, isSuccess, refetch } = useQuery(
    "fetchCategoriesList",
    getTaxReturnDocuments
  );
        ...
        <div className="flex flex-col lg:w-[45%] gap-[0.5rem] w-full mb-6 px-10">
          ...
          <div
            className={`flex flex-row text-[0.9rem] font-[600] group-focus-within:bg-white  group-focus-within:shadow-md border  ${
              !errors.taxYearReturn ? "border-[#AFBACA]" : "border-red-600 "
          ...
          >
            <input
              ...
              {...register("taxYearReturn", {
                required: "*This field is required.",
              })}
            />
          </div>
          {errors.taxYearReturn?.type === "required" && (
            <p className="text-red-600 text-sm">{errors.taxYearReturn.message}</p>
          )}
        </div>
